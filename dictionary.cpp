...
int main() {
    ...
    ifstream dictFile("C:\\Users\\aagam\\practice\\dictionary.txt");
    string word;
    ...
    vector<string> correctedWords; // Store corrected words
    ...
    while (ss >> word) {
        if (!searchWord(root, word)) { // If word is misspelled,
            ...
            vector<string> suggestions = getSuggestions(root, word); // Get suggestions for misspelled word

            if (!suggestions.empty()) {
                cout << " \nSuggestions:";
                for (int i = 0; i < suggestions.size(); i++) {
                    cout << " (" << i << ") " << suggestions[i];
                }
                ...
                if (choice >= 0 && choice < suggestions.size()) {
                    correctedWords.push_back(suggestions[choice]);
                    cout << suggestions[choice];
                } else {
                    // User chose an invalid option, keep the original word
